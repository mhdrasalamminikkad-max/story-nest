...
try {
  if (!admin.apps.length) {
    // Initialize Firebase Admin with environment variables or hardcoded defaults
    const projectId = process.env.FIREBASE_PROJECT_ID || process.env.VITE_FIREBASE_PROJECT_ID || "story-7af93";
    const clientEmail = process.env.FIREBASE_CLIENT_EMAIL || "firebase-adminsdk-fbsvc@story-7af93.iam.gserviceaccount.com";
    ...
    if (projectId && clientEmail && privateKey) {
      // Use service account credentials from environment variables or defaults
      admin.initializeApp({
      ...
      console.warn("⚠ Firebase Admin credentials not found. Authentication will not work.");
      console.warn("  Please provide: FIREBASE_PROJECT_ID, FIREBASE_CLIENT_EMAIL, FIREBASE_PRIVATE_KEY");
    }
  }
